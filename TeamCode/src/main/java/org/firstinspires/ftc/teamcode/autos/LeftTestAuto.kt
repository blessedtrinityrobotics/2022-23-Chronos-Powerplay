...
class LeftTestAuto : LinearOpMode() {
    ...
    private fun scoreTargetCone(){
        ...
        while(drivetrain.imu.angle.firstAngle > (baseAngle + TARGET_CONE_ANGLE) && opModeIsActive()){
            drivetrain.tankDrive(-0.2,0.2)
        }
        drivetrain.tankDrive(0.0,0.0)
        drivetrain.encoderReset()
        ...
        while (drivetrain.imu.angle.firstAngle < (oldAngle + PARKING_ANGLE) && opModeIsActive()){
            drivetrain.tankDrive(0.2,-0.2)
        }
         drivetrain.tankDrive(0.0,0.0)
        drivetrain.encoderReset()
